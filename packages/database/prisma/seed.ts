/**
 * Seed: Super Admin + permission catalog + starter roles.
 * Run: npx prisma db seed (from packages/database) after migrate.
 */
...
async function main() {
  ...
  const allKeys = allPerms.map((p) => p.id);
  ...
  for (const permissionId of allKeys) {
  ...
  // Starter operational roles (empty or minimal — Super Admin configures)
  const starterRoles = [
